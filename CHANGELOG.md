...
- c88a173: bench: the rust engine (our main flavor) can now boot. `score --engine rust`
...
- 0969bfe: Glow up the `smoo auth login` browser callback page in the Presence design
...
- 63971a5: Daemon session refresh no longer revokes its own Smoo session (th-c6a542).
...
- c79226a: Big Smooth can now text a group and name a number.
...
- 67494e1: CI: format-check every PR with oxfmt, the repo's single formatter
...
- f8aa3eb: Re-green the pre-commit hook: `cargo clippy --workspace --all-targets` (what
...
- 73f0518: Bump smooai-smooth-operator-core 1.7.0 → 1.7.2 so streaming turns read the
...
- b37fcc5: th-mail: read the mailbox you actually registered, and fail loudly when the store is broken.
...
- 9f92b27: Desktop OTA 0.1.11 — proving release: publishes the stable `latest-arm64.dmg`
  download alias (SMOODEV-2961 homepage button) and exercises the OTA
  self-install (stale-daemon guard #456 + await-daemon-exit swap #457) end to end
  on the fleet.
- b0ce9f0: OTA publish now also writes a stable `latest-arm64.dmg` alias to
  downloads.smoo.ai so the homepage "Download for Mac" button (SMOODEV-2961)
  has a durable target that never needs a version bump.

## 0.35.10

### Patch Changes

- 2b07f44: Release-hardening: add an LLM-driven WS smoke suite that drives the real Big
...
- de89536: Big Smooth SPA: queue messages typed while a turn is in flight. Instead of blocking the send (the old `turn-guard`), a message typed mid-turn is held in a client-side queue and sent automatically — one at a time, in order — when the active turn's reply lands. Queued messages render as removable chips above the composer with a "Clear queued" affordance that empties the queue without interrupting the running turn (distinct from Stop, which cancels only the turn). Idle sends are unchanged.

## 0.35.4

### Patch Changes

- f63bc9a: Desktop app OTA 0.1.10 — ships the session's merged work to the installed Big Smooth app: Plan⇄Auto modes + shift+tab / chip, Cmd+N new chat, the rich searchable model selector with capability badges, the fixed "Enable notifications" button + notify tool, and the safety-judge controls. Version bump so electron-updater serves it (it compares versions).

## 0.35.3

### Patch Changes

- d34521e: Give the user control over Big Smooth's safety judge (the Narc hook).
...
- 3614111: bench: separate safety breaches from completeness misses, cost tokens instead of a shared key, and stop scoring filtered turns
...
- 1c92958: Agent mail becomes MCP tools every harness can reach, plus an optional cloud backend.
...
--setup-statusline` wires it up and never overwrites an existing `statusLine`;
  ...
  agents on _different machines_ share one bus — the one thing local SQLite cannot
...
- 267782a: `th` CLI rough-edge sweep — exhaustive dispatch, readable help, scriptable output.
