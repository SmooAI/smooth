...
literally "Big Smooth, _the user's own_ always-on assistant"; the filesystem and
shell tools are confined to _the owner's_ `~/dev`; `recall`/`remember` is _the
owner's_ memory; the bundled MCP tokens (Outlook, calendar) are _the owner's_
...
The load-bearing question is **identity**. A persona a client merely _asserts_
...
1. **A `Principal` threaded into the permission gate.** `PermissionRules`
   becomes _per-principal_: the daemon holds a role → rules map and selects by
   the verified identity of the frame's origin, not a global set. The matcher
   language and fail-safe `Ask` default are unchanged — only the _selection_ is
...
owner _and delegated principals_. What holds the line:
...
2. **Deny-by-default per principal.** The gate's fail-safe `Ask`/deny default
   now applies _per identity_: a principal with no ruleset can do nothing. A new
...
3. **The existing three layers stay, now keyed on identity.** Permission gate →
   Narc → kernel sandbox all run per call as today; the change is _which_ rules
   and _which_ Narc profile, selected by verified principal. The kernel sandbox
...
- **Local personas on the parent's daemon (PIN/passkey-gated switch).** Ships
  ...
  client bypasses it. Acceptable as a _demo milestone that already plumbs
  per-principal RBAC keyed on an injected identity_ (so the org model is a
...
- `ToolHook` sees only `&ToolCall` (no principal), and the two security hooks are
  process-global singletons. So the permission _hook_ cannot branch on caller —
...
Jr's **no-egress** guarantee comes from denying every egressing _tool_ at this
...
doesn't. M2.5 adds the next grain down — **which calendars** a role that _does_
have the tool may see and touch — the first _within-tool resource_ allowlist.
...
- **Enforcement** (`smooth_tools::calendar`, the `ical` seam): reads
  ...
  names, and calendar _management_ (create/rename/delete) is refused. An empty
...
**Still deferred** (unchanged from above): M3 per-principal _memory_ partition
