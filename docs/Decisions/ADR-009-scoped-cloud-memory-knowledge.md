...
1. **Memory is local, unsynced, unscoped.** `operator_storage`'s memory is one
   ...
   the access context and *discards* it). Nothing syncs across a user's faces
...
2. **Platform knowledge is org-only.** api.smoo.ai knowledge is fully built —
   ...
   scoped to `organization_id` and nothing finer for a *member*. There is no
...
3. **The ask**: cloud-synced memory and knowledge that can be **personal**,
   **family-shared**, or **org-shared**; children's data special-cased; and
   parents able to share only *certain* local calendars with a child.
...
- `owner_user_id = <user>` → **personal**: visible only to that user, synced
  across *their own* faces/devices.
...
"family" scope. On the knowledge side the shared tier *already exists* (it is
...
special accounts" is encoded as *asymmetric visibility*, not a separate store.
...
entitlement check is a gate on *whether the daemon talks to the cloud tier*, not
a new failure mode: local is always the floor.

### 4. Within-tool resource RBAC — calendars are the first case

ADR-008 RBAC is **tool-level** (a role gets `calendar` or not). Sharing *only
certain* calendars with a child is **within-tool resource** RBAC: the child gets
...
- Enforcement: `SandboxedToolProvider::tools_for` already knows the principal's
  ...
  **injects the allowlist into that tool instance for the turn**; the tool filters
  ...
  Narc-visible. The allowlist is bound from the *principal*, not passed by the
...
- **Family and org as distinct scopes.** ADR-008 already made a family *be* an
...
- Two enforcement points per read, because api-prime runs on a `BYPASSRLS` pool:
  the **RLS policy** (for direct Supabase access) *and* the **app-layer filter**
...
- Memory recall quality *improves* as a side effect of Phase B: the platform's
...
- Guardian visibility is blocked on platform family roles (th-95eff9); shipping
  ...
  default; the guardian clause only *widens* parent reads later).
...
- **Memory attribution** — a `source`/author column so a shared fact records
  *who taught it* ("Mom said the wifi password is …"); trust + audit under
...
- **Child → shared-memory writes go through approval**, not straight in — route a
  child's proposed *shared* write to a parent review queue (reuse the Hermes
...
- **Child-safety Narc on memory content** — the per-role child-safety detectors
  ADR-008 already deferred, now also gating what gets *remembered*.
...
- **Connected-account personal memory** — Gmail/calendar MCP feeding
  *personal*-scoped auto-memory ("remember my dentist appointments").
