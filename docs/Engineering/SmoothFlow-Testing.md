...
|                | `hooks_answer_without_the_daemon_token_and_everything_else_is_gated` | 401 without the token on every route but hooks; `?token=`, Bearer, `X-Smooth-Token`; the WS handshake refuses a bad token                                                                                                                                                                                                                                                        |
|                | `forged_hooks_move_nothing_on_a_live_session`                        | th-91d032: no token, another session's token, a made-up token and a pre-resume token move nothing on a live agent. A forged PermissionRequest is answered `{}` at once, not held. The current token still drives state.                                                                                                                                                          |
