...
`sidebar.closeOut.<id>` (the row's context-menu item — the Session menu item
shares its title, so the tests key on this identifier), `session.close.live` /
`session.close.dirty` (what the sheet says it will destroy),
`session.close.refusal` / `session.close.force` / `session.close.keep`,
