...
**`push` / `pull` are notices.** Both exit 0 and print where sync is headed (`th pearls sync` against Smoo Projects work items — pearl th-19cca5), because hooks still call them. `log`, `remote`, `gc`, `doctor`, `migrate-from-beads` and `migrate-from-dolt` are gone with Dolt.
