...
| -------------------------- | ------------------------------------------------- | -------------------------------------------------------------------- |
| **Local pearl tracking**   | `th pearls …`                                     | SQLite at `~/.smooth/pearls.db`, rows scoped by project              |
| **Jira sync**              | `th jira sync`                                    | Atlassian REST + the pearl store                                     |
...
**`th pearls sync` — two-way sync with Smoo Projects (pearl th-19cca5).** The local db stays the read path; sync is the explicit reconcile against the same work-items API `smoo work` wraps, authenticated as you (`smoo auth login`). Bind a checkout once with `th pearls sync --project <KEY|uuid>` (stored in the pearl store's per-project config); after that a bare `th pearls sync` does both directions. `--pull-only` / `--push-only` restrict it, `--dry-run` reports what would change without writing either side, `--json` emits the report, `--org` overrides the active org.

- **Identity**: pearl ↔ work item pairs live in the store's `sync_map` table, and every pushed item carries `externalRef = th-xxxxxx@<checkout-dir-name>`, so a second machine adopts existing items by id instead of duplicating them (and keeps the pearl id when it is free).
- **Field map**: title, description, labels; status `open`→`open`, `in_progress`→`in_progress`, `closed`→`done`, `deferred`→`blocked` (pull also folds `in_review`→`in_progress`, `cancelled`→`closed`, and never demotes a remote `in_review`/`cancelled` that already means the same thing locally); priority is **inverted** (pearl P0-critical = work item 4-highest); type `epic` has no remote twin and survives as the `epic` label. Deps become `blocks` links (`blocker --blocks--> blocked`); comments mirror both ways, ours prefixed with a `pearl-comment:<id>` first line so they are not pulled back as echoes. Assignees and scheduling are not synced.
- **Last writer wins** by `updated_at` against the baseline recorded at the last sync. Both sides changed → the newer one wins and the loser is _listed_ under `conflict:` (never silent). Create-on-first-sight both ways; **nothing is ever deleted** — a pearl or item that vanished on one side is reported under `skipped:` and left alone on the other.
- Deps and comments are reconciled for mapped pearls that are open or were touched this run (two GETs each); closed history is not re-swept.
- **Jira**: the platform owns it now — `smoo work jira import` pulls Jira into work items, and `th pearls sync` brings them down. `th jira sync` (smooth-diver's direct client) still works but is the legacy path.

**`push` / `pull` are notices.** Both exit 0 and point at `th pearls sync`, because hooks still call them. `log`, `remote`, `gc`, `doctor`, `migrate-from-beads` and `migrate-from-dolt` are gone with Dolt.
...
Besides the health checks (daemon, pearl store, providers, `~/.smooth`, pearls, backup,
