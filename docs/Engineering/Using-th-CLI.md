...
**Migrating a legacy `.smooth/dolt` store.** `th pearls migrate-from-dolt [PATH]` imports every table (pearls, deps, labels, comments, history, memories, config) preserving ids and timestamps. Pearls upsert by `updated_at` and everything else is insert-or-ignore, so it is safe to re-run; the Dolt directory is left in place for you to delete. It needs the `smooth-dolt` binary — the last thing that does, until pearl th-c6ba83 removes the shim.

**`push` / `pull` are notices.** Both exit 0 and print where sync is headed (`th pearls sync` against Smoo Projects work items — pearl th-19cca5), because hooks still call them. `log`, `remote`, `gc`, `doctor` and `migrate-from-beads` are gone with Dolt.
