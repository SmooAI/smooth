...
└── harness/                       # per-harness overlays — never inside the composed plugin
    ...
    ├── cursor/hooks.json          # Cursor's flat {"version":1} hooks, merged into ~/.cursor/hooks.json
    ├── gemini|qwen|droid/hooks.json  # hook-only: merged into that CLI's settings.json (th-b00115)
    ├── copilot/hooks.json         # hook-only: → ~/.copilot/hooks/<pkg>.json
    ├── amp/plugin.ts              # hook-only: → ~/.config/amp/plugins/<pkg>.ts
    ├── pi/extension.ts            # hook-only: → ~/.pi/agent/extensions/<pkg>.ts
...
| **gemini, qwen, droid** | **Hook-only** (th-b00115): `harness/<h>/hooks.json` key-merged into the CLI's own `settings.json` (`~/.gemini/`, `~/.qwen/`, `~/.factory/`); every other key in that file is left alone. Marker dirs `~/.gemini`, `~/.qwen`, `~/.factory`.                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **copilot**             | **Hook-only**: `harness/copilot/hooks.json` → `~/.copilot/hooks/<pkg>.json`, a file of its own (Copilot reads every `*.json` there), `${CLAUDE_PLUGIN_ROOT}` substituted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **amp, pi**             | **Hook-only**: `harness/amp/plugin.ts` → `~/.config/amp/plugins/<pkg>.ts`, `harness/pi/extension.ts` → `~/.pi/agent/extensions/<pkg>.ts` (symlinks).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
