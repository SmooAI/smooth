...
Use `smoo api smooth-operator chat` when the ask is an org operation, e.g.:
...
subcommands already do (`th pearls`, `th worktree`, `smoo api agents`, …).

## Auth

Smooth Operator routes are user-authed. Run `smoo auth login` once (Supabase user
...
smoo api smooth-operator chat "Find contacts named Jane and draft a follow-up email"

# Continue it — pass the conversationId from the previous turn
smoo api smooth-operator chat "Make it warmer" --conversation <conversation-id>

# Machine-readable turn result (conversationId, reply, toolCalls, pendingAction)
smoo api smooth-operator chat "..." --json

# Read a conversation back
smoo api smooth-operator history <conversation-id>
```
