...
Code identifiers, env vars (`SMOOTH_SAFEHOUSE_MODE` / `_PORT` / `_IMAGE`), file names, tracing fields, and current-state docs all flip. `SMOOTH_BOARDROOM_IMAGE` remains accepted as a fallback in the CLI's env resolution to avoid breaking ad-hoc scripts; the published OCI image keeps its `ghcr.io/smooai/boardroom:latest` tag until republished, and the CLI's bind-mount overlay places the renamed `safehouse` binary at the image's legacy entrypoint path so the runtime rename is complete today.
