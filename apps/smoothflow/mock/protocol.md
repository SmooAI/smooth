...
- **HTTP** siblings for one-shot calls: `GET /api/flow/sessions`, `POST /api/flow/sessions`, `POST /api/flow/sessions/{id}/input`, `POST /api/flow/sessions/{id}/resize`, `POST /api/flow/sessions/{id}/approve`, `POST /api/flow/sessions/{id}/kill`, `POST /api/flow/hooks` (see Hooks).
