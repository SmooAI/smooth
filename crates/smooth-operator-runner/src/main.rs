...
impl Tool for TodoListTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<String> {
        ...
        match action {
            ...
            "add" => {
                let items = args.get("items").and_then(|v| v.as_array()).ok_or_else(|| anyhow::anyhow!("action='add' requires 'items' array"))?;
                if items.is_empty() {
                    return Err(anyhow::anyhow!("action='add' requires at least one item"));
                }
                for item in items {
                    let text = item.get("text").and_then(|v| v.as_str()).ok_or_else(|| anyhow::anyhow!("each item must have 'text'"))?;
            ...
            "update" => {
                let id = args.get("id").and_then(serde_json::Value::as_u64).ok_or_else(|| anyhow::anyhow!("action='update' requires integer 'id'"))?;
                let status = args.get("status").and_then(|v| v.as_str()).ok_or_else(|| anyhow::anyhow!("action='update' requires 'status'"))?;
...
async fn main() {
    ...
    // Pearl th-67e338: hard directive about tool invocation. Small
    // coding models (qwen3-coder-flash, some open-weights variants)
    // default to emitting pseudo-XML function blocks in plain text
    // instead of using the API's native tool_calls field. The runtime
    // CANNOT execute those — the call is silently dropped and the
    // task stalls. State the rule loudly so the model picks the
    // right channel.
    let tool_invocation_note = "\n\n## CRITICAL: tool invocation\n\nTo run any tool (`read_file`, `write_file`, `edit_file`, `apply_patch`, `list_files`, `bash`, `grep`, etc.), invoke it through the API's native tool_calls mechanism. The runtime ONLY executes tool calls delivered that way.\n\n**Do NOT** emit pseudo-XML like `<function=read_file>path</function>` or `<tool_call>{...}</tool_call>` in your message body. Those are plain text — the runtime cannot see them and will NOT run them. The agent will appear to be talking about doing work while accomplishing nothing.\n\nWhen you need to read a file, edit a file, run a shell command, search the codebase, etc., emit a real tool_call. Do not narrate the call in prose first; just make it.";
    ...
    let base_prompt = format!(
        "{provider_overlay}{}{pearl_note}{hints_note}{tool_invocation_note}\n\n## Agent Role\n\n{}",
...
#[cfg(test)]
mod todo_list_tests {
    use super::{Tool, TodoListTool};
    use serde_json::json;
    use tempfile::TempDir;

    fn tool() -> (TodoListTool, TempDir) {
        let dir = TempDir::new().expect("tempdir");
        let t = TodoListTool { base: dir.path().to_path_buf() };
        (t, dir)
    }
    ...
    async fn persists_across_tool_instances() {
        ...
        let t1 = TodoListTool { base: dir.path().to_path_buf() };
        t1.execute(json!({"action": "add", "items": [{"text": "Survives across turns"}]})).await.expect("add");
        let t2 = TodoListTool { base: dir.path().to_path_buf() };
