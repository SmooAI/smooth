//! Safehouse mode.
//!
//! When `SMOOTH_SAFEHOUSE_MODE=1`, Big Smooth boots with its own cast:
...
//!   Smooth's tool surface inside the Safehouse is empty; included for
//!   parity so future features inherit surveillance automatically).
//! * **Scribe** — structured logging. Forwards to the safehouse's own
//!   Archivist so safehouse activity is visible alongside operator logs.
//! * **Archivist** — **central log aggregator**. Bound on `0.0.0.0:4401`
//!   inside the Safehouse VM so Bill can port-forward it to a host port,
...
//! # Lifecycle
//!
//! [`spawn_safehouse_cast`] is called once from [`crate::server::start`]
//! when `SMOOTH_SAFEHOUSE_MODE=1`. It spawns every cast member on its own
//! tokio task and returns a [`SafehouseHandles`] struct holding the live
//! URLs. The handles outlive every incoming request — they are kept in
//! `AppState` so future code can query `safehouse.archivist_url()` etc.
...
/// Live URLs of every Safehouse cast member. Stored on `AppState` so
/// request handlers can talk to their in-process neighbours (and tests
/// can assert on them).
#[derive(Clone)]
pub struct SafehouseHandles {
    /// URL of the Safehouse's own Wonk. `http://127.0.0.1:<port>`.
    pub wonk_url: String,
    /// URL of the Safehouse's own Goalie. `http://127.0.0.1:<port>`.
    pub goalie_url: String,
    /// URL of the Safehouse's own Scribe. `http://127.0.0.1:<port>`.
    pub scribe_url: String,
    /// URL of the Safehouse's Archivist. **Bound on 0.0.0.0:4401** (the
    /// fixed guest port Bill maps to a host port on Safehouse spawn).
    /// `http://0.0.0.0:4401`.
    pub archivist_url: String,
    /// Archivist's bound port (always 4401 in Safehouse mode).
    ...
    /// URL of the Safehouse's Diver (pearl lifecycle manager).
    pub diver_url: String,
}

impl std::fmt::Debug for SafehouseHandles {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SafehouseHandles")
...
impl SafehouseHandles {
    /// Archivist URL as seen from inside another VM. Operator VMs see the
    /// Safehouse Archivist at `http://host.containers.internal:<host_port>`,
    /// where `<host_port>` is whatever Bill mapped 4401 to when spawning
    /// the Safehouse. We don't know that host port from inside the
    /// Safehouse process; the caller (the test harness / bootstrap code)
    ...
    pub fn operator_facing_archivist_url(&self) -> Option<String> {
        ...
        // The operator VM needs to reach the Safehouse's Archivist via
...
/// The fixed guest port Archivist binds on inside the Safehouse VM.
/// Bill maps it to an ephemeral host port when spawning the Safehouse.
pub const ARCHIVIST_GUEST_PORT: u16 = 4401;

/// Spawn every Safehouse cast member on its own tokio task.
///
/// # Errors
///
/// Returns an error if any HTTP bind fails (port in use, permission
/// denied, etc.) or if the default policy TOML cannot be parsed.
pub async fn spawn_safehouse_cast(pearl_store: Option<smooth_pearls::PearlStore>) -> Result<SafehouseHandles> {
    tracing::info!("safehouse: spawning in-process cast");
    ...
    let archivist_listener = tokio::net::TcpListener::bind(("0.0.0.0", ARCHIVIST_GUEST_PORT))
        .await
        .with_context(|| format!("safehouse: bind archivist on 0.0.0.0:{ARCHIVIST_GUEST_PORT}"))?;
    let archivist_addr = archivist_listener.local_addr().context("safehouse: archivist local addr")?;
    let archivist_url = format!("http://{archivist_addr}");
    tokio::spawn(async move {
        if let Err(e) = axum::serve(archivist_listener, archivist_r).await {
            tracing::error!(error = %e, "safehouse: Archivist server crashed");
        }
    });
    tracing::info!(url = %archivist_url, "safehouse: Archivist up");

    // --- Scribe (forwards to our own Archivist) ----------------------------
    // When Big Smooth runs inside the Safehouse VM, its own Scribe uses
    // the loopback archivist URL — same process, same memory, but goes
    // through the HTTP path for consistency with operator Scribes.
    let scribe_forwarder = spawn_forwarder(archivist_url.clone(), "safehouse".to_string());
    let scribe_state = ScribeState::with_forwarder(scribe_forwarder.clone());
    let scribe_r = scribe_router(scribe_state);
    let scribe_listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.context("safehouse: bind scribe")?;
    let scribe_addr = scribe_listener.local_addr().context("safehouse: scribe local addr")?;
    let scribe_url = format!("http://{scribe_addr}");
    tokio::spawn(async move {
        if let Err(e) = axum::serve(scribe_listener, scribe_r).await {
            tracing::error!(error = %e, "safehouse: Scribe server crashed");
        }
    });
    tracing::info!(url = %scribe_url, "safehouse: Scribe up");

    // --- Wonk -------------------------------------------------------------
    // The safehouse Wonk is minimal: we ship a default-permissive policy
    // so the legal policy file format is exercised and any future
    // safehouse tool surface gets guardrails for free. Policy can be
    ...
    .context("safehouse: generate default wonk policy")?;
    let policy = smooth_policy::Policy::from_toml(&default_policy_toml).map_err(|e| anyhow::anyhow!("safehouse: parse wonk policy: {e}"))?;
    ...
    let wonk_listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.context("safehouse: bind wonk")?;
    let wonk_addr = wonk_listener.local_addr().context("safehouse: wonk local addr")?;
    let wonk_url = format!("http://{wonk_addr}");
    tokio::spawn(async move {
        if let Err(e) = axum::serve(wonk_listener, wonk_r).await {
            tracing::error!(error = %e, "safehouse: Wonk server crashed");
        }
    });
    tracing::info!(url = %wonk_url, "safehouse: Wonk up");

    // --- Goalie -----------------------------------------------------------
    // Goalie is a full HTTP forward proxy. We give it a WonkClient pointed
    // at the safehouse Wonk and let `run_proxy` bind the port itself.
    let goalie_audit_path = std::env::var("SMOOTH_SAFEHOUSE_GOALIE_AUDIT").unwrap_or_else(|_| "/tmp/goalie-safehouse.jsonl".into());
    let goalie_audit = smooth_goalie::AuditLogger::new(&goalie_audit_path).context("safehouse: create goalie audit logger")?;
    let goalie_wonk_client = smooth_goalie::WonkClient::new(&wonk_url);
    let goalie_listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.context("safehouse: probe goalie port")?;
    let goalie_addr = goalie_listener.local_addr().context("safehouse: goalie local addr")?;
    ...
    tokio::spawn(async move {
        if let Err(e) = smooth_goalie::run_proxy(&goalie_listen, goalie_wonk_client, goalie_audit).await {
            tracing::error!(error = %e, "safehouse: Goalie proxy crashed");
        }
    });
    tracing::info!(url = %goalie_url, audit = %goalie_audit_path, "safehouse: Goalie up");
    ...
    let diver_url = if let Some(store) = pearl_store {
        ...
        let diver_listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.context("safehouse: bind diver")?;
        let diver_addr = diver_listener.local_addr().context("safehouse: diver local addr")?;
        let url = format!("http://{diver_addr}");
        tokio::spawn(async move {
            if let Err(e) = axum::serve(diver_listener, diver_r).await {
                tracing::error!(error = %e, "safehouse: Diver server crashed");
            }
        });
        tracing::info!(url = %url, "safehouse: Diver up");
        url
    } else {
        tracing::warn!("safehouse: no PearlStore provided, Diver not started");
        String::new()
    };
...
mod tests {
    ...
    #[tokio::test]
    async fn spawn_safehouse_cast_brings_up_every_member() {
        // Skip if 4401 is already bound (concurrent test, or stale
        // safehouse process). This is a smoke test, not a contention
        // regression guard.
        ...
        let handles = match spawn_safehouse_cast(None).await {
            Ok(h) => h,
            Err(e) => {
                eprintln!("skipping: safehouse cast spawn failed: {e}");
                return;
            }
        };
