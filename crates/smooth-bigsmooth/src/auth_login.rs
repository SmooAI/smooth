...
//!    file `th`'s user-authed API calls read).
//! 4. `GET /api/auth/status` — the UI polls this to render "Signed in as
//!    …" vs a sign-in button.
//!
//! ponytail: duplicated from smooth-cli auth::{browser_login,pkce};
//! extract to a shared crate if a third consumer appears. `smooth-bigsmooth`
//! can't depend on `smooth-cli` (the CLI depends on the lib, not vice-versa).
//!
//! Pearl th-bc624a.
...
/// smoo.ai's `/cli-login`.
pub async fn login_handler(State(state): State<AppState>, headers: HeaderMap) -> Response {
    state.touch();
...
pub async fn callback_handler(State(state): State<AppState>, RawQuery(query): RawQuery) -> Response {
    state.touch();
    let query = query.unwrap_or_default();

    // Cheap pre-check: surface an upstream `error=` (e.g. the user
    // cancelled) as a friendly page. We still validate state below via
    // the pending lookup, so this can't be used to skip CSRF.
    // (parse_callback also handles this, but we need the state first to
    // do the single-use removal regardless.)
