...
pub async fn run_with_session(
    ...
    // HITL approvals: the old auto-mode SSE subscriber tailed the bespoke
    // :4400 `/api/access/stream`, which the operator never feeds — so it was
    // dead on the operator path. Removed with the :4400 nuke; the operator's
    // `write_confirmation_required` / `confirm_tool_action` flow replaces it
    // (th-1ea4f6).
