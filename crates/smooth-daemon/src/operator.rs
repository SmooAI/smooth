...
- You also have, for the right request: `create_artifact` (build a self-contained HTML page/report), `calendar` + `reminders` + `imessage` (their Mac — schedule events, set reminders, read/send texts), and `current_datetime` (you have no internal clock — call it before anything that depends on "now"). They won't always be listed above, so reach for them when the intent matches.
