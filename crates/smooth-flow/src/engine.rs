...
mod tests {
    ...
    fn harness_prefs_persist_order_and_hide() {
        ...
        // The first four built-ins lead; later built-ins (th-b00115) follow in registry order.
        let head = |v: Vec<String>, n: usize| v.into_iter().take(n).collect::<Vec<_>>();
        let all = e.harnesses(true).unwrap();
        assert_eq!(head(names(&all), 4), ["claude", "opencode", "codex", "th-code"]);
        let total = all.len();
        let mut rx = e.subscribe();
        let all = e.set_harness_prefs(Some(vec!["th-code".into()]), Some(vec!["codex".into()])).unwrap();
        assert_eq!(head(names(&all), 4), ["th-code", "claude", "opencode", "codex"]);
        assert_eq!(all.len(), total);
        assert!(all[3].hidden);
        let visible = |v: Vec<String>| v.into_iter().filter(|n| n != "codex").collect::<Vec<_>>();
        match rx.try_recv().unwrap() {
            ServerFrame::Harnesses { harnesses } => {
                assert_eq!(head(names(&harnesses), 3), ["th-code", "claude", "opencode"]);
                assert_eq!(names(&harnesses), visible(names(&all)));
            }
            other => panic!("{other:?}"),
        }
        // Visible list + hello omit the hidden one; a partial PUT keeps the other half.
        assert_eq!(names(&e.harnesses(false).unwrap()), visible(names(&all)));
        match e.hello().unwrap() {
            ServerFrame::Hello { harnesses, .. } => assert_eq!(names(&harnesses), visible(names(&all))),
            other => panic!("{other:?}"),
        }
