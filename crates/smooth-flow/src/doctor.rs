//! Harness doctor (pearl th-3cabf6) — does each harness actually work on
//! THIS machine? The core behind `th harness doctor` (which only prints it)
//! and the degraded badge in every SmoothFlow picker (th-51bf88, see
//! [`crate::Engine::harnesses`]).
//!
...
use crate::harness::{Manifest, Registry, StateSource};
...
impl Verdict {
    /// The wire spelling (the `snake_case` serde name).
    #[must_use]
    ...
    /// How `th harness doctor` prints it.
    #[must_use]
...
impl Diagnosis {
    /// What a picker shows for it (th-51bf88).
    #[must_use]
    pub fn health(&self) -> crate::harness::HarnessHealth {
...
pub struct Machine {
    ...
    pub keychain: bool,
    /// Check that a daemon is listening for hooks. Off when the daemon IS the
    /// caller: it is, by definition.
...
impl Machine {
    ...
    /// This machine as the flow daemon sees it (th-51bf88). The daemon's own
    /// environment IS the one sessions launch from (the app's, when the app
    /// started it), so its `PATH` is checked directly and no app `PATH` is
    /// simulated, and it does not probe for itself.
    #[must_use]
    pub fn daemon(home: PathBuf) -> Self {
        Self {
            app_path: None,
            check_daemon: false,
            ..Self::current(home)
...
fn flow_hook_is_authenticated(script: &Path) -> bool {
    std::fs::read_to_string(script).is_ok_and(|t| t.contains(crate::hook_auth::TOKEN_FILE_ENV))
}
    ...
    for rel in rels {
            ...
            .find_map(|f| std::fs::read_to_string(f).ok().filter(|t| t.contains(marker)).map(|t| (f, t)))
        {
            ...
            let stale = if marker == "flow-hook.sh" {
                ...
                (!text.contains(crate::hook_auth::TOKEN_FILE_ENV)).then(|| hit.display().to_string())
            };
...
fn opencode_hooks(m: &Machine) -> Check {
    ...
    if !text.contains(crate::hook_auth::TOKEN_FILE_ENV) {
        return check(
            "hooks",
            Level::Fail,
            format!(
...
fn installed_checks(manifest: &Manifest, m: &Machine, bin: &Path, checks: &mut Vec<Check>) -> (Option<PathBuf>, Option<String>) {
    ...
    match manifest.state.source {
        ...
        StateSource::Native if m.check_daemon => checks.push(daemon_check(m)),
        StateSource::Native => {}
...
mod tests {
    ...
    /// th-51bf88: inside the daemon there is no daemon to look for, and the
    /// daemon's own PATH is the app's, so neither check is simulated.
    #[cfg(unix)]
    #[test]
    fn the_daemon_machine_skips_the_self_probe_and_the_simulated_app_path() {
        let m = Machine::daemon(PathBuf::from("/nonexistent-home"));
        assert!(!m.check_daemon);
        assert!(m.app_path.is_none());
        assert!(Machine::current(PathBuf::from("/h")).check_daemon);

        let tmp = tempfile::tempdir().unwrap();
        let bin = tmp.path().join("bin");
        script(&bin.join("mynative"), "#!/bin/sh\necho 'mynative 1.0'\n");
        let man = Manifest::parse(
            "name = \"mynative\"\n[binary]\nnames = [\"mynative\"]\n[launch]\nargv = [\"{prompt}\"]\nprompt_as = \"argv\"\n[state]\nsource = \"native\"\n",
        )
        .unwrap();
        let mut m = machine(tmp.path(), &[&bin], None);
        assert!(diagnose(&man, &m).checks.iter().any(|c| c.id == "signal"), "the CLI checks for a daemon");
        m.check_daemon = false;
        let d = diagnose(&man, &m);
        assert!(!d.checks.iter().any(|c| c.id == "signal"), "the daemon does not: {d:#?}");
        assert_eq!(d.verdict, Verdict::Works);
        let h = d.health();
        assert_eq!((h.verdict.as_str(), h.reason, h.fix), ("works", None, None));
    }

    #[test]
    fn health_carries_the_verdict_reason_and_fix_on_the_wire() {
        let tmp = tempfile::tempdir().unwrap();
        let d = diagnose(&manifest("claude"), &machine(tmp.path(), &[&tmp.path().join("empty")], None));
        let v = serde_json::to_value(d.health()).unwrap();
        assert_eq!(v["verdict"], "not_installed");
        assert_eq!(v["fix"], "curl -fsSL https://claude.ai/install.sh | bash");
        assert!(v["reason"].as_str().unwrap().contains("not found"));
        for verdict in [Verdict::Works, Verdict::Degraded, Verdict::NotInstalled] {
            assert_eq!(serde_json::to_value(verdict).unwrap(), verdict.as_str(), "as_str is the serde name");
        }
    }
